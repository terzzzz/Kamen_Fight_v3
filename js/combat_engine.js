    ...
    if (move1.type === 'DEFENSE') {
      ...
      if (result.isOffensive) {
        if (move2.type === 'DEFENSE' && !defender1.isFainted) {
          ...
          // GLANCING HIT / SCRATCH: Play Hit animation (not dodge) & DO NOT award Chi Gain
          if (typeof window.playCenterVideo === 'function') {
...
function resolveAttack(attacker, defender, atkMove, atkMoveKey, defMove, defMoveKey, defenderKey) {
  ...
  if (defender.isFainted) {
    ...
    rolledHit = true;
  } else if (!defMove || defMove.type === 'IDLE' || defKeyStr === 'DO_NOTHING' || defMove.name === 'Do Nothing') {
    rolledHit = true;
  } else {
    let baseHitChance = atkMove.hitChance || 80;
...
async function executeTurnResolutionPhase() {
  ...
  let p1IsS = typeof p1MoveKey === 'string' && p1MoveKey.startsWith('S');
  let p2IsS = typeof p2MoveKey === 'string' && p2MoveKey.startsWith('S');
  let p1IsD = typeof p1MoveKey === 'string' && p1MoveKey.startsWith('D');
  let p2IsD = typeof p2MoveKey === 'string' && p2MoveKey.startsWith('D');
