...
async function executeTurnResolutionPhase() {
  ...
  // Round conclusion & post-turn checks
  setTimeout(() => {
...
function confirmPlayerAction(moveKey, playerKey = 'p1') {
  ...
  // Capture charge meter for BOTH Human and CPU
  const capturedCharge = (player.chargeState && typeof player.chargeState.currentPercent === 'number' && player.chargeState.currentPercent > 0)
    ? player.chargeState.currentPercent
    : (player.activeChargePercent !== undefined ? player.activeChargePercent : 100);
  ...
  if (playerKey === 'p1' && (!window.gameState.input || !window.gameState.input.isConfirmed)) {
    ...
    window.gameState.p1.activeChargePercent = moveKey === 'DO_NOTHING' ? 100 : capturedCharge;
    ...
    window.gameState.p2.activeChargePercent = moveKey === 'DO_NOTHING' ? 100 : capturedCharge;
  ...
  // NOTE: Auto-chain reaction trigger for CPUs removed here so each CPU charges independently to its own target!
...
function startBattle(matchConfig) {
  ...
  try {
    try {
      ...
      window.gameState.p1Moves = typeof window.FALLBACK_ICHIGO_MOVES !== 'undefined' ? window.FALLBACK_ICHIGO_MOVES : {};
      window.gameState.p2Moves = typeof window.FALLBACK_ICHIGO_MOVES !== 'undefined' ? window.FALLBACK_ICHIGO_MOVES : {};
    }

    const p1Rider = matchConfig.p1Rider || { id: 'ichigo', name: 'Kamen Rider Ichigo', maxLp: 2300, sourceFacing: 'left' };
    const p2Rider = matchConfig.p2Rider || { id: 'nigo', name: 'Kamen Rider Nigo', maxLp: 2500, sourceFacing: 'right' };
